...
function scoreFromResponse(response: string): number {
  ...
  const hasStructure = /because|therefore|first|second|risk|mitigation|plan/i.test(normalized);
...
function buildFallbackEvaluation(response: string): DemoEvaluation {
  const score = scoreFromResponse(response);
  return {
    score,
    feedback: 'Live AI evaluation is temporarily unavailable, so this score is generated by local demo logic. Keep answers structured with concrete trade-offs and metrics.',
    strengths: [
      'You provided a direct response to the scenario.',
      'Your answer can be improved by making assumptions explicit.',
      'Use measurable outcomes to strengthen decision quality.',
    ],
    weaknesses: [
      'Missing explicit downside risks and mitigation steps.',
      'Could include clearer prioritization criteria.',
      'Needs more numeric targets for execution confidence.',
    ],
  };
}

function fallbackScenario(roundNumber: number): DemoScenario {
  if (roundNumber === 2) {
    return {
      context: 'Customer Trust and Reliability',
      question: 'Your first 20 users report flaky automations after integrating multiple tools. Renewals are in 10 days. What is your immediate strategy?',
      options: [
        { id: 'A', text: 'Pause new signups and fix reliability for current users first.', feedback: 'Prioritizes trust but slows growth.' },
        { id: 'B', text: 'Keep growth campaigns running while a small team works on stability.', feedback: 'Balances growth but risks continued churn.' },
        { id: 'C', text: 'Offer refunds and migrate users to a simplified stable workflow.', feedback: 'Reduces support load but impacts short-term revenue.' },
      ],
    };
  }

  return {
    context: 'Go-To-Market Pressure',
    question: 'You are building a lower-cost n8n-style workflow platform. A bigger competitor just dropped prices by 30%. What do you do next?',
    options: [
      { id: 'A', text: 'Double down on a niche segment with prebuilt templates and faster onboarding.', feedback: 'Strong differentiation if niche demand is real.' },
      { id: 'B', text: 'Match competitor pricing immediately across all plans.', feedback: 'May improve conversions but compresses margins.' },
      { id: 'C', text: 'Introduce a usage-based plan with premium support upsell.', feedback: 'Can align value with spend but increases pricing complexity.' },
    ],
  };
}

function fallbackFollowup(optionText: string): DemoFollowupScenario {
  return {
    context: 'Second-order consequences',
    question: `After choosing "${optionText}", your support queue grows 2x and onboarding slows. How will you stabilize operations in the next 14 days without hurting retention?`,
  };
}

function fallbackPitch(roundNumber: number): { question: string; context: string } {
  if (roundNumber > 0) {
    return {
      context: `Investor Q&A Round ${roundNumber}`,
      question: `What are the top 2 assumptions behind your growth model, and what evidence in the next 30 days would prove or disprove each one?`,
    };
  }

  return {
    context: 'Investor pitch',
    question: 'Give a 90-second founder pitch: problem, why now, your wedge, and why your team can win.',
  };
}

function fallbackNegotiation(): { question: string; context: string } {
  return {
    context: 'Commercial negotiation',
    question: 'A strategic customer asks for a 40% discount and a custom integration. What terms would you accept, and what do you ask for in return?',
  };
}

function fallbackCompetencyReport(): any {
  return {
    overallScore: 72,
    summary: 'The report is running in local demo fallback mode due to temporary AI unavailability.',
    entrepreneurType: 'Execution-Focused Operator',
    organizationalRole: 'Founding CEO / Product Operator',
    archetypeNarrative: 'You make practical decisions quickly and can improve outcomes by making risks and success metrics explicit earlier in each response.',
    competencies: [
      { code: 'DECISION', trait: 'Decision Quality', score: 74, category: 'Strategy' },
      { code: 'CLARITY', trait: 'Communication Clarity', score: 70, category: 'Leadership' },
      { code: 'EXECUTION', trait: 'Execution Planning', score: 73, category: 'Operations' },
    ],
    roleFitMap: {
      role: 'Early-Stage Builder',
      bestEnvironment: 'Seed to Series A companies with fast iteration cycles.',
      dominantCompetencies: ['Decision Quality', 'Execution Planning'],
    },
    actionPlan: [
      { competency: 'Decision Quality', action: 'Add one risk and one mitigation to every strategic answer.', targetDate: 'Within 7 days' },
      { competency: 'Communication Clarity', action: 'Use a 3-part structure: context, decision, expected outcome.', targetDate: 'Within 14 days' },
      { competency: 'Execution Planning', action: 'Attach one KPI and one review checkpoint to each plan.', targetDate: 'Within 14 days' },
    ],
  };
}

async function extractApiError(res: Response): Promise<{ message: string; status: number }> {
  ...
  const rawMessage = payload?.error || payload?.message || `Request failed with status ${status}`;
  return { message: String(rawMessage), status };
}

function isRetryable(status: number, message: string): boolean {
  if (RETRYABLE_STATUS.has(status)) return true;
  return /timeout|temporar|unavailable|overloaded|rate limit/i.test(message);
}

function toUserSafeError(message: string): string {
  if (/503|unavailable|high demand|overloaded/i.test(message)) {
    return 'AI is temporarily overloaded. The app automatically retried, but the service is still unavailable. Please try again shortly.';
  }
  return message;
}
...
async function postJsonWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const { message, status } = await extractApiError(res);
      lastError = message;
  ...
  throw new Error(toUserSafeError(lastError));
}

async function apiGenerateScenario(introduction: string, roundNumber: number, previousScenarios: string): Promise<DemoScenario> {
  return postJsonWithRetry('/demo/generate-scenario', { introduction, roundNumber, previousScenarios }, () => fallbackScenario(roundNumber));
}
...
async function apiGenerateFollowup(
  ...
  return postJsonWithRetry(
    '/demo/generate-followup',
    { introduction, originalQuestion, selectedOptionText, selectedOptionFeedback, roundNumber },
    () => fallbackFollowup(selectedOptionText)
  );
}

async function apiEvaluateText(introduction: string, question: string, response: string): Promise<DemoEvaluation> {
  return postJsonWithRetry(
    '/demo/evaluate-response',
    { introduction, question, response },
    () => buildFallbackEvaluation(response)
  );
}
...
async function apiEvaluateVoice(introduction: string, question: string, audio: Blob): Promise<DemoVoiceEvaluation> {
  ...
  let lastError = 'Failed to evaluate voice';
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      lastError = err?.message || 'Network error while evaluating voice';
      if (attempt < MAX_RETRIES) {
        await sleep(500 * Math.pow(2, attempt - 1));
        continue;
      }
  ...
  throw new Error(`${toUserSafeError(lastError)} If this keeps happening, switch to text mode for this round.`);
}

async function apiGeneratePitch(introduction: string): Promise<{ question: string; context: string }> {
  return postJsonWithRetry('/demo/generate-pitch', { introduction }, () => fallbackPitch(0));
}
...
async function apiGeneratePitchQnA(introduction: string, pitchResponse: string, roundNumber: number): Promise<{ question: string; context: string }> {
  return postJsonWithRetry(
    '/demo/generate-pitch-qna',
    { introduction, pitchResponse, roundNumber },
    () => fallbackPitch(roundNumber)
  );
}
...
async function apiGenerateNegotiation(introduction: string, pitchResponse: string, roundNumber: number, previousContext: string): Promise<{ question: string; context: string }> {
  return postJsonWithRetry(
    '/demo/generate-negotiation',
    { introduction, pitchResponse, roundNumber, previousContext },
    () => fallbackNegotiation()
  );
}

async function apiGenerateCompetencyReport(summary: string): Promise<any> {
  return postJsonWithRetry('/demo/generate-competency-report', { summary }, () => fallbackCompetencyReport());
}
